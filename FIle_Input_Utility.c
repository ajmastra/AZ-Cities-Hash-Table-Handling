    ...
    int clearLeadingWhiteSpace( FILE *filePtr, bool clearSpace )
       {
        ...
        && ( ( charInt < (int)SPACE || charInt > (int)HIGHEST_PRINTABLE_CHAR )
       ...
       if( checkForInputFileOpen() && !checkForEndOfInputFile() )
          {
           ...
           // otherwise, assume non printable not cleared
            ...
            // AND
            // continues if not printable flag set
            // and characters (including TAB) are printable
            // OR not printable flag not set
            && ( ( stopAtNonPrintable 
                           && ( intChar >= (int)SPACE || intChar == (int)TAB ) )
                   || ( !stopAtNonPrintable ) )
